import { NavLink, Outlet } from "react-router-dom";
import { Copyright, ShoppingCart, User } from "lucide-react";
import Floating from "./Floating";
import useCarrinho from "../hooks/useCarrinho";
import { useState } from "react";
import CartModal from "./CartModal";

const navLinks = [
  { label: "Home", path: "/" },
  { label: "Cardápio", path: "/menu" },
  { label: "Meus Pedidos", path: "/pedidos" },
  { label: "Sobre Nós", path: "/sobre" },
  { label: "Fale Conosco", path: "/contato" },
];

export default function Layout() {
  const carrinho = useCarrinho();
  const [isCartOpen, setCartOpen] = useState(false);

  const toggleCartOpen = () => {
    setCartOpen(!isCartOpen);
  };

  return (
    <div className="bg-[#622B14] flex flex-col text-slate-50 min-h-screen">
      <header className="flex justify-between gap-10 p-5 text-lg font-playfair">
        <h1>Backery</h1>
        <nav className="flex gap-10">
          {navLinks.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
              className={({ isActive }) => `${isActive && "font-bold"}`}
            >
              {item.label}
            </NavLink>
          ))}
        </nav>
        <User />
      </header>

      <main className="flex-1 bg-[#f7efe6] text-black">
        <Outlet />
      </main>

      <footer className="flex flex-col items-center p-2 bottom-0">
        <div className="flex items-end gap-1">
          <h2 className="text-2xl font-bold font-playfair">Bakery</h2>
          <Copyright />
        </div>
        <p className="text-sm italic">Faz parte do seu dia a dia!</p>
      </footer>

      {carrinho.itens.length !== 0 && (
        <Floating
          onClick={() => {
            toggleCartOpen();
          }}
        >
          <ShoppingCart />
        </Floating>
      )}

      {isCartOpen && <CartModal isOpen={isCartOpen} onClose={toggleCartOpen} />}
    </div>
  );
}
